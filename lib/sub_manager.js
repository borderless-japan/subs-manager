SubsManager = function (options) {
  ...
  self.subscribeFunc = self.options.subscribe || Meteor.subscribe && _.bind(Meteor.subscribe, Meteor)
...
SubsManager.prototype._addSub = function(args) {
  ...
  if (args.length > 0 && _.isObject(args[0])) {
    hashArgs = args.slice(1);
  } else {
    hashArgs = args;
  }
  var hash = EJSON.stringify(hashArgs);
  if(!self._cacheMap[hash]) {
    ...
    // console.log(['cache miss, will rerun', hash]);
    self._reRunSubs();
  }
...
SubsManager.prototype._reRunSubs = function() {
  var self = this;
  // console.log(['rerun', self.computation.stopped, self.computation]);
...
SubsManager.prototype._registerComputation = function() {
  ...
  var computation = Deps.autorun(function() {
    // console.log('subsman fire');
    self._applyExpirations();
    self._applyCacheLimit();

    var ready = true;
    _.each(self._cacheList, function(sub) {
      sub.ready = self.subscribeFunc.apply(null, sub.args).ready();
